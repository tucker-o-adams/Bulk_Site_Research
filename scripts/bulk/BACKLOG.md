...
| 9d | **Operating workflow for the tool** (noted 2026-09-23; **draft v0.1 written: `WORKFLOW.md`**, pending Tucker's review; not yet shared with Mike). A defined sequence for running a portfolio end to end, with thresholds set *before* any site is assessed as an explicit step (see 9b) — e.g. intake → product/target-MW definition → thresholds approved → run → flags → survivors → parcels/exhibits. **Draft our own hypothesis of this workflow before sharing anything with Mike**; the thresholds template goes to him as part of it, not on its own | — | — |
