...
Ranking: owner first (an ArcGIS Online organisation whose name looks like the
county or state beats a well-titled layer owned by an individual), then layer
name, then whether the fields a parcel needs are actually present, then whether
a probe point returns exactly one polygon.
"""
